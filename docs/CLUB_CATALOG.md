...
- `id`는 PostgreSQL `clubs` 행의 영구 기본 키다. 표시명 변경이 필요해도 ID를 재사용하거나 임의 변경하지 않는다.
...
위 정본 레코드에 다음 필드를 공통으로 합성한다. 가격·발행량 등 시장 값은 종목별로 작성하지 않고 `market_config/current`의 동일한 개발 기본값을 적용한다.
...
`ratingCount: 0`이면 UI는 3.0점으로 표시하지 않고 **평가 없음**으로 표시한다. 표시용 number와 별도로 가격 엔진은 정수 milli-star 필드를 사용한다. 가격·`fundamentalPrice` 등 시장값은 이 JSON에 종목별로 반복하지 않고 공통 `market_config/current`에서 합성한다. 로고·이미지·부스 정보와 운영 시간은 외부 정보가 제공될 때까지 `null`을 유지한다.
