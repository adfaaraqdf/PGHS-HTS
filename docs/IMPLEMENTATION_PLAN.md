...
### 생성·수정 파일
`AGENTS.md`, `docs/PRODUCT_REQUIREMENTS.md`, `docs/ARCHITECTURE.md`, `docs/DATABASE_SCHEMA.md`, `docs/SECURITY_MODEL.md`, `docs/DATA_CONTRACTS.md`, `docs/CLUB_CATALOG.md`, `docs/ETF_STRUCTURE.md`, `docs/PRICE_ENGINE.md`, `docs/IMPLEMENTATION_PLAN.md`, `docs/TEST_PLAN.md`, `docs/DEPLOYMENT_PLAN.md`, `docs/FESTIVAL_OPERATIONS.md`, `docs/DECISIONS.md`.

### 구현 범위
기능·비기능 요구, 데이터 흐름, 테이블, RLS 경계, 가격·ETF·랭킹 정책, 0~12단계, 시험·배포·현장 운영·장애 복구·비용·폐장 계획을 문서화한다. 20개/6개/소속·별칭을 교차 검증한다.

### 구현하지 않을 범위
애플리케이션 코드, Supabase 리소스 생성, 패키지 설치, 시드 실행, Git commit/push, 배포.
...
## 1단계 — 프로젝트 기반과 Supabase 연결

### 목표
Vite 기반 Vanilla 앱 셸, Supabase SDK 초기화, PostgreSQL migration·Database Functions 기반, static hosting·local stack·환경 분리를 만든다.

### 선행 조건
0단계 승인, 개발 Supabase 프로젝트 또는 Supabase local stack 사용 방침, Node/Supabase CLI 버전 결정.

### 생성·수정 파일
`package.json`, lockfile, `vite.config.js`, `index.html`, `src/main.js`, `src/styles/`, `src/config/{env,supabase}.js`, `src/router/`, `src/views/`, `src/components/`, `supabase/config.toml`, `.env.example`, 기본 테스트·린트 설정.

### 구현 범위
로그인·홈·시장·ETF·내 자산·랭킹·관리자 자리 화면, 모바일 하단 내비게이션, 360px 앱 셸, 공통 loading/empty/error/retry 상태, Auth/PostgreSQL/Database Functions 핸들 초기화, Supabase local stack 연결, 개발·테스트·운영 환경 검증을 구현한다.

### 구현하지 않을 범위
로그인 동작, 데이터 조회, 거래, 가격, 공식 카탈로그 하드코딩, 관리자 기능, 다음 단계 비즈니스 로직.

### 데이터 흐름
브라우저 시작 → 환경 스키마 검증 → Supabase client/Auth 초기화 → 현재 route의 자리 화면 렌더 → 화면 해제 시 Realtime 채널 정리.

### 보안 고려사항
Web config와 서버 secret을 구분하고 실제 secret·서비스 계정 키를 저장소/번들에 넣지 않는다. 환경 누락은 fail-fast하며 production이 Supabase local stack에 연결되거나 반대가 되지 않게 한다.

### 테스트 방법
`npm ci`, lint, unit, production build, Database Functions 정적 검사, Supabase local stack 기동을 검증한다. 360px·대표 모바일 폭, 키보드 포커스, 44px 터치, console error 0, 환경 누락 오류, secret scan을 확인한다.

### 완료 조건
문서화된 명령이 새 clone에서 재현되고 일곱 자리 화면을 이동할 수 있으며 실제 기능은 연결되지 않았다.

### 롤백 방법
스캐폴드와 Supabase 설정을 이전 태그로 되돌린다. 생성된 원격 리소스가 없어야 하며 로컬 Supabase local stack 데이터만 삭제한다.
...
### 선행 조건
1단계 통과, `ALLOWED_SCHOOL_DOMAIN`의 Supabase local stack 값, 사용자·공개 프로필 계약, 닉네임 기본 정책.

### 생성·수정 파일
`src/services/auth.js`, 로그인/닉네임/거부 화면, auth state 모듈, `supabase/migrations/*_functions.sql`의 `initialize_user`, 공통 검증·오류 모듈, Auth/Database RPC local stack 테스트와 관련 문서.

### 구현 범위
모바일 Google OAuth, 확인 이메일, UX용 도메인 검사, `initialize_user`의 `auth.users` provider·도메인·계정 상태 재검사, 트랜잭션 기반 idempotent create, 최초 현금 1,000,000원, 서버 시각, 재로그인 `last_login_at`, 안전한 닉네임 검증을 구현한다.

### 구현하지 않을 범위
학생 명부 없는 실제 학생 복수 계정 차단, 거래, 전체 RLS 완성, 관리자 UI.

### 데이터 흐름
Google OAuth → Supabase session → `initialize_user(p_nickname)` → `auth.users` email/provider/domain 검증 → UID 행 transaction get-or-create → private account/public profile 최소 필드 → 클라이언트 세션.

### 보안 고려사항
클라이언트는 UID·현금·이메일을 전달하지 않는다. `initialize_user`가 `auth.uid()`와 `auth.users`의 provider·확인 이메일·중앙 학교 도메인을 검사한다. 이메일은 `accounts`에 복제하지 않으며 RLS는 활성 계정과 소유자를 검사한다.

### 테스트 방법
허용 최초/재로그인, 초기화 전 접근 거부, non-Google provider, 외부 도메인, 미인증 이메일, 연속·동시 초기화, 중간 실패 재시도, logout/token 만료, 잘못된 닉네임, 현금·행 중복 없음 테스트를 local stack에서 자동화한다.

### 완료 조건
동시 재호출 후에도 UID 문서 1개·초기 현금 1회이며 거부 계정은 데이터에 접근하지 못한다.

### 롤백 방법
새 로그인 진입을 feature flag로 막고 이전 Auth/RPC 버전으로 되돌린다. 이미 생성된 사용자는 삭제하거나 현금을 재지급하지 않는다.

## 3단계 — PostgreSQL 스키마, RLS, 공식 시드

### 목표
데이터 계약, deny-by-default RLS, 인덱스, 공정한 공식 20종목·6ETF·시장 설정 시드를 구현한다.

### 선행 조건
2단계 통과, 스키마·RLS·카탈로그·ETF·개발 기본값 승인.

### 생성·수정 파일
`supabase/migrations/`, `supabase/seed.sql`, 정본 데이터 모듈, `supabase/tests/`, `supabase/config.toml`, RPC 공유 validator와 관련 문서.

### 구현 범위
문서화된 accounts/holdings/trade_history/public_profiles/clubs/etfs/private trades·trade_requests/ratings/news/admin_events/audit_logs/leaderboard/market/price·demand 구조와 필수 인덱스를 migration으로 만든다. `supabase/seed.sql`은 local reset 기본이며 `ON CONFLICT DO NOTHING`으로 비파괴 재실행된다. production 적용은 대상 project ref 확인과 별도 승인이 필요하다.

### 구현하지 않을 범위
거래 RPC, 가격 계산, 실제 별점, 임의 동아리·콘텐츠, 운영 데이터 자동 덮어쓰기.

### 데이터 흐름
정본 JSON/모듈 → schema validation·관계 검증 → 대상 프로젝트 확인 → create/skip 또는 명시적 허용 필드 update → 생성·건너뜀·갱신 보고. 클라이언트 읽기/쓰기 → RLS의 auth/domain/owner/role/type/field/query 제한.

### 보안 고려사항
현금·보유·원장·가격·거래량·별점·ETF·랭킹·시장·감사 로그의 클라이언트 권위 쓰기를 차단한다. 타인 private read, 추가 필드, 잘못된 타입·음수·소수, 감사 로그 수정·삭제를 차단한다.

### 테스트 방법
요구된 공격 RLS matrix 전체와 인덱스 query를 Supabase local stack에서 검증한다. 20/6/고유성/정확히 1 ETF/alias 비중복/공통 초기값/재실행/비강제 덮어쓰기 차단을 자동 검증한다.

### 완료 조건
RLS·seed 테스트와 빌드가 통과하고 공식 데이터 외 항목이 없으며 일반 클라이언트가 권위 필드를 쓸 수 없다.

### 롤백 방법
RLS·인덱스는 직전 검증 버전으로 재배포한다. Supabase local stack 시드는 재생성하고 운영 시드는 생성 manifest·백업을 기준으로 승인된 신규 문서만 제거한다. `--force`로 복구하지 않는다.
...
### 선행 조건
3단계 RLS/seed 통과, U-008 거래 상대방·발행 수량 의미 확정, 시장 상태·오류·평균가 계약 승인.

### 생성·수정 파일
`supabase/migrations/*_functions.sql`의 `buy_stock`, `sell_stock`, `private.execute_trade`, 수요 샤드 테이블, 클라이언트 거래 service·상태 UI, 단위/통합/local stack 동시성 테스트, 관련 계약 문서.

### 구현 범위
RPC buy/sell, `{clubId,quantity,idempotencyKey}` 검증, 계정/시장/종목 상태·서버 가격·현금/보유 확인, 평균가, user/holding/history/global trade/request/demand shard 원자 갱신, 안전한 오류 코드, 버튼 중복 잠금·재시도 UX를 구현한다.

### 구현하지 않을 범위
클라이언트 가격·총액 신뢰, 지정가·호가·주문 매칭, 가격 변경, 공매도·파생상품·송금·사용자 간 양도.

### 데이터 흐름
클라이언트 UUID → RPC → auth/domain/account/market/club 검증 → request·user·holding·club 읽기 → 서버 가격 계산 → 단일 transaction 쓰기 → 안정 오류/체결 응답 → 화면 최신 스냅샷. 수요 샤드는 요청 해시로 10개 중 하나를 선택한다.
...
### 롤백 방법
즉시 전체 시장 halt 후 거래 RPC을 직전 버전으로 되돌린다. 부분 데이터를 직접 수정하지 않고 request/trade/audit 원장으로 진단한 승인 repair만 수행한다.
...
### 생성·수정 파일
`supabase/migrations/*_functions.sql`의 `private.run_price_tick`, Supabase Cron 설정, 가격 history/집계 RLS·index, 시뮬레이션·golden/property·local stack 테스트, 모니터링 쿼리.
...
### 보안 고려사항
service role 전용 쓰기, 설정 변경 관리자·감사 로그, tick lease/ID로 중복 실행 차단, 입력·설정 해시로 재현성을 보장한다.
...
### 롤백 방법
시장을 halt하고 scheduler를 비활성화한 뒤 직전 설정/RPC으로 되돌린다. 마지막 정상 tick 이후 잘못된 가격은 감사 가능한 복구 작업으로만 복원하고 거래가 있었다면 임의 되감지 않는다.
...
### 데이터 흐름
route enter → 제한 query/listener 등록 → view model/정수 formatter → 화면 → 거래 RPC → 성공 시 authoritative snapshot 반영 → route leave 시 unsubscribe/abort.
...
### 롤백 방법
static hosting을 이전 UI release로 되돌린다. 서버 계약은 하위 호환을 유지하며 긴급 시 거래를 halt하고 읽기 전용 화면을 제공한다.
...
### 생성·수정 파일
ETF 계산/자료화 RPC 또는 서버 모듈, `src/views/etf.js`, ETF components/service, RLS·index·단위/E2E 테스트.
...
### 완료 조건
ETF 수·구성과 계산이 정본과 일치하고 거래 UI/RPC가 존재하지 않는다.
...
### 생성·수정 파일
랭킹 RPC·private entries·public snapshot RLS/index migration, `src/views/ranking.js`, ranking service/components, privacy·tie·부하 테스트.
...
### 생성·수정 파일
관리자 authenticated RPC, `src/views/admin.js`, admin services/components, claim provisioning 도구, news/events/market/audit RLS·index와 보안/E2E 테스트.
...
### 구현하지 않을 범위
클라이언트 직접 service role, 무감사 수정·삭제, 첫 실거래 후 초기화, 이벤트 상한 우회, 임의 현금·가격 편집.

### 데이터 흐름
admin UI → RPC → token claim·계정·역할·rate limiting and abuse monitoring·입력 검증 → target+append-only audit 원자 write → public sanitized listener → 가격 엔진은 시간상 활성 event만 읽기.
...
### 롤백 방법
관리자 쓰기 기능을 feature flag로 끄고 시장 halt 후 이전 RPC/UI로 되돌린다. 감사 로그는 보존하고 잘못된 이벤트는 취소 이벤트로 상쇄한다.
...
### 생성·수정 파일
Supabase Edge Function 또는 Database Webhook 기반 ratings adapter, 확정 시 제출 RPC와 평가 UI/공식 링크, Vault secret/feature flag 설정, ratings 집계·dead-letter/monitoring, fixture/contract/integration/security 테스트와 운영 문서.
...
### 데이터 흐름
학생 평가(지원 시) → 인증 RPC → provider 계약 또는 공식 이동; provider webhook/poll → 서명·schema·idempotency → canonical event/aggregate → `ratings(club_id)` 원자 갱신 → 가격 tick이 닫힌 snapshot 읽기. 실패는 quarantine·alert·재대조한다.

### 보안 고려사항
secret은 Supabase project secrets 또는 Vault, replay 방지, 허용 source·rate limit·payload limit, 최소 PII, 원본 로그 redaction을 적용한다.
...
### 생성·수정 파일
폐장 orchestration/admin Database Functions, final snapshot 스키마·RLS, 폐장/결과 UI, 감사·복구 도구, 상태 머신·E2E·failure injection 테스트.
...
### 구현 범위
동시 500명 임시 기준과 1.5배 확정 부하, hotspot·listener·auth ramp, RLS/OWASP/rate limiting and abuse monitoring/프로젝트 역할 및 CI 접근 제어/secret 검토, 비용 산정, 백업·rollback·시장 halt·폐장 리허설, staging canary와 go/no-go 자료를 완성한다.
...
### 보안 고려사항
가짜 계정·비식별 데이터만 사용하고 least-privilege 프로젝트 역할 및 CI 접근 제어, dependency/secret scan, RLS negative test, rate limiting and abuse monitoring 모니터→enforce, 로그 redaction, 관리자 claim 회수를 검증한다.
